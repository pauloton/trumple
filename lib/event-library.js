...
export const FEATURED_SECOND_TERM_EVENTS = Object.freeze(
  mergeLibrary(SEED_EVENTS, [...CURATED_NEWS_EVENTS, ...EXPANDED_CURATED_EVENTS, ...GENERATED_EVENTS, ...WEEKLY_CURATED_EVENTS])
);
...
export function weeklyEventsForSunday(sunday, events = SECOND_TERM_EVENTS) {
  ...
  const candidates = events
    .filter((event) => event.status !== "candidate" && event.dateBasis !== "article-publication" && event.date >= start && event.date <= end)
    .sort((a, b) => b.significance - a.significance || b.date.localeCompare(a.date) || a.id.localeCompare(b.id));

  // Spread the puzzle across the week, then fill with distinct stories from
  // busy days. Same-day cards are interchangeable when the player submits.
  const byDate = new Map();
  for (const event of candidates) if (!byDate.has(event.date)) byDate.set(event.date, event);
  const selected = new Map([...byDate.values()].map((event) => [event.id, event]));
  for (const event of candidates) {
    if (selected.size >= 7) break;
    selected.set(event.id, event);
  }
  return [...selected.values()].sort((a, b) => a.date.localeCompare(b.date) || a.id.localeCompare(b.id));
}
