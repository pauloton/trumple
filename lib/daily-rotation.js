...
export function createDailyRotationSelector(events, {
  ...
  const buildDay = (day) => {
    ...
    const recentEligible = ordered.filter((event) => day - ordinal(event.date) <= rules.recentWindowDays);
    ...
    addUntil(recentEligible, isRecent, rules.recentPerPuzzle);
    ...
    // has enough distinct dates, the stronger rules above always satisfy a day.
